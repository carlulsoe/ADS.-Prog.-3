...
public class MazePair {
    public int shortestPath(char[][] maze) {
        ...
        for (int i = 1; i < height; i++) {
            for (int j = 1; j < width; j++) {
                if (maze[i][j] == 'r') {
                    shortest = Math.min(path(maze.clone(), i, j, 0), shortest);
                }
            }
        }
    ...
    private int path(char[][] maze, int i, int j, int val) {
        ...
        if (maze[i][j] == 'b') {
            ...
            maze[i][j] = (char) (val + 1);
            if (maze[i][j+1] == '.' || maze[i][j+1] == 'b') {
                out = Math.min(path(maze, i, j + 1, val + 1), out);
            }
            if (maze[i][j-1] == '.' || maze[i][j-1] == 'b') {
                out = Math.min(path(maze, i, j-1, val+1), out);
            }
            if (maze[i+1][j] == '.' || maze[i+1][j] == 'b') {
                out = Math.min(path(maze, i + 1, j, val + 1), out);
            }
            if (maze[i-1][j] == '.' || maze[i-1][j] == 'b') {
                out = Math.min(path(maze, i - 1, j, val + 1), out);
            }
        ...
        else if (maze[i][j] != 'o' && maze[i][j] != 'b') {
            maze[i][j] = (char) Math.min(val + 1, (int) (maze[i][j]));
            if (maze[i][j+1] != 'o' && maze[i][j+1] != 'r' && maze[i][j+1] != 'b' && maze[i][j+1] > val + 1) {
                out = Math.min(path(maze, i, j + 1, val + 1), out);
            }
            if (maze[i][j-1] != 'o' && maze[i][j-1] != 'r' && maze[i][j-1] != 'b' && maze[i][j-1] > val + 1) {
                out = Math.min(path(maze, i, j-1, val+1), out);
            }
            if (maze[i+1][j] != 'o' && maze[i+1][j] != 'r' && maze[i+1][j] != 'b' && maze[i+1][j] > val + 1) {
                out = Math.min(path(maze, i + 1, j, val + 1), out);
            }
            if (maze[i-1][j] != 'o' && maze[i-1][j] != 'r' && maze[i-1][j] != 'b' && maze[i-1][j] > val + 1) {
                out = Math.min(path(maze, i - 1, j, val + 1), out);
