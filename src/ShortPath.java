...
public class ShortPath {
    private HashMap<Integer, Integer> vertex = new HashMap<>();
    ...
    public int shortestPath(Edge[] edges, int nodeCount, int maxEdges, int sourceNode, int targetNode) {
        ...
        int i = sourceNode;
        int k = 0;
        while (parents.get(i) != null && i != targetNode && k < maxEdges) {
            sum += vertex.get(i);
            i = parents.get(i);
            k++;
        }
    ...
    private void ISS(int nodes, int sourceNode) {
        for (int i = 0; i < nodes; i++) {
            vertex.put(i, 100000);
            parents.put(i, null);
        }
        vertex.put(sourceNode, 0);
        distance.put(sourceNode, 0);
    }
    private void relax(int u, int v, Edge w) {
